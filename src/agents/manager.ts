...
export async function createManagerAgent(runId: string = randomUUID(), options: { sessionId?: string } = {}) {
  ...
  const quickHelp = await createQuickHelpAgent();
  const chat = await createChatAgent();
  const post = await createPostAgent();
  const image = await createImageAgent();
  const research = await createDocsResearchAgent(runId);
  const automationPlanner = await createAutomationPlannerAgent();
  const personalAssistant = await createPersonalAssistantAgent();
  const developerHelper = await createDeveloperHelperAgent(runId);
  const security = await createSecurityAgent(runId);
  const coding = await await createCodingAgent(runId);
  const goalManager = await createGoalManagerAgent();
  const finance = await createFinanceAgent(runId);
